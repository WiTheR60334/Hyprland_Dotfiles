...
if [ ! -f ~/.config/hypr/.initial_startup_done ]; then
    ...
    flatpak override --user --env=XCURSOR_THEME=Banana
    flatpak override --user --env=XCURSOR_SIZE=40
	...
	if [ -n "$(grep -i nixos < /etc/os-release)" ]; then
      ...
      dconf write /org/gnome/desktop/interface/cursor-size "40" > /dev/null 2>&1 &
	fi
